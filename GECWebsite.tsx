...
import gecConfig from '../config/gec-2025.ts'
// Alexis Lewis worked on this file
export default function GECWebsite() {
  const { eventInfo, branding, homepage, schedule, speakers, registration, sponsors, contact } = gecConfig
  ...
  return (
    <div style={{
      ...
      <section style={{ padding: "3rem 1rem", backgroundColor: branding.primaryColor, color: "#ffffff" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "1.2rem", marginBottom: "2rem" }}>
            Early Bird Deadline: {new Date(registration.earlyBirdDeadline).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
          </p>
          <div style={{
            display: "grid",
            gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))",
            ...
            {registration.registrationTypes.map((type, index) => (
              <div key={index} style={{
                backgroundColor: "#ffffff",
                color: "#000000",
                padding: "2rem",
                ...
                <div style={{ marginBottom: "1rem" }}>
                  <div style={{ fontSize: "0.9rem", marginBottom: "0.5rem" }}>
                    Early Bird: <strong>${type.prices.earlyBird}</strong>
                  </div>
                  <div style={{ fontSize: "0.9rem", marginBottom: "0.5rem" }}>
                    Regular: <strong>${type.prices.regular}</strong>
                  </div>
                  <div style={{ fontSize: "0.9rem" }}>
                    Onsite: <strong>${type.prices.onsite}</strong>
                  </div>
                </div>
      ...
      <section style={{ padding: "3rem 1rem", backgroundColor: "#f8fafc" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
            <strong>Email:</strong> <a href={`mailto:${contact.email}`} style={{ color: branding.primaryColor }}>{contact.email}</a>
          </p>
          <p style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
            <strong>Phone:</strong> {contact.phone}
          </p>
          <p style={{ fontSize: "1rem", color: "#64748b" }}>
            {contact.organizingCommittee}
          </p>
        </div>
      </section>
      ...
      <footer style={{
        ...
        <p style={{ marginBottom: "1rem" }}>{gecConfig.footer.copyright}</p>
        <div style={{ display: "flex", gap: "2rem", justifyContent: "center", flexWrap: "wrap" }}>
          {gecConfig.footer.quickLinks.map((link, index) => (
            <Link
              key={index}
              to={link.path}
              style={{
